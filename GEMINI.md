...
12. **4x4 模块化 AI 报告系统:** 侧边栏实时生成结构化 Markdown 报告。支持 4 种风格与 4 种需求的 16 种自由组合。配置修改（风格、侧重、排盘种类）会触发报告即时更新。
13. **三方四正与大一统 AI 指令:** 自动组装紫微“三方四正”数据。采用“大一统 Prompt 架构”，动态合并专家身份与解盘逻辑集，支持多术合参（八字+紫微+占星）并保持序号连续。
14. **可选排盘种类:** 用户可自由勾选“八字”、“紫微”、“星座”的一种或多种组合。UI 界面与 AI 报告内容将根据勾选状态实时同步条件渲染。针对移动端，排盘种类选择器被置于命盘顶端以便于实时切换。
...
20. **流年全息太岁雷达 (Tai Sui Radar):** 动态推演流年当值太岁，并在年、月、日、时四柱中全息扫描“值、冲、害、破、刑”等高危关系。特别针对日柱（夫妻宫/自身）受冲时，会触发最高级别的 💥 警告，并在 AI 报告中注入极具干预性的防范指令。
21. **全屏沉浸向导:** 交互逻辑重构为两步闭环向导流程。在进入第二步（出生地校正）时，第一步的表单将自动折叠为简约预览文本（同步展示公农历生日、生肖、时间及性别），并提供一键返回修改功能。在点击排盘后将彻底隐藏输入表单，进入全屏专注阅览模式。
