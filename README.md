...
- **专业辅助功能**：
    - **真太阳时自动修正**：内置覆盖全国的省市经度库，支持基于经度偏移与均时差（EOT）的自动校时。
    - **时辰边界提醒**：显示当前时辰的起始与结束时间点及修正参考值，辅助精确判断。
    - **经度实时展示**：选择城市后即刻显示经纬度，方便核对数据准确性。
    - **专业术语悬浮简介**：鼠标悬停在十神、纳音、上升星座上即可查看详细定义的 Tooltips，排版精美且支持多行显示。
    - **Markdown 报告导出**：实时生成排版精美的 Markdown 格式报告，包含八字/六字对照行，支持一键复制。
- **极致隐私与便携**：
